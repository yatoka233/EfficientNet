...
def main(args):
    ...
    if os.path.exists(args.weights):  ## 如果权重文件存在
    ...
    # Scheduler https://arxiv.org/pdf/1812.01187.pdf  余弦下降法
    lf = lambda x: ((1 + math.cos(x * math.pi / args.epochs)) / 2) * (1 - args.lrf) + args.lrf  # cosine 倍率下降
    ...
    for epoch in range(args.epochs):
        print("learning rate: ", round(optimizer.param_groups[0]["lr"], 5))
        # train
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.01)  ## lr 初始值
    parser.add_argument('--lrf', type=float, default=0.01) ## lr 最终缩小倍率

    # 数据集所在根目录
    # http://download.tensorflow.org/example_images/flower_photos.tgz
    parser.add_argument('--data-path', type=str,
                        default="archive/seg_train/seg_train")
